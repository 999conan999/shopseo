...
if($id_post==-1){
     $sql = $wpdb->prepare( "SELECT id_post,rs_comment,rs_user_name,rs_phone,rs_status,rs_rep,json_img FROM $table_prefix WHERE rs_status ='private' ORDER BY id DESC ");
   $results = $wpdb->get_results( $sql , OBJECT );
}else{
     $sql = $wpdb->prepare( "SELECT id_post,rs_comment,rs_user_name,rs_phone,rs_status,rs_rep,json_img FROM $table_prefix WHERE id_post = %d ORDER BY id DESC ",$id_post);
     $results = $wpdb->get_results( $sql , OBJECT );
}
   $rs=array();
