...
function render_xml_first(){
    ...
    $sql = $wpdb->prepare( "SELECT id_post,title,short_des,price,thumnail,img_shoping,shoping_type,instock,id_category FROM $table_prefix WHERE shoping_on_off = 'on' AND post_status = 'publish' ORDER BY id_category DESC ");
    ...
    if(count($results)>0){
      ...
      foreach($results as $x){
          ...
          $values['gtin'] = "";
          $values['mpn'] = "";
        if($x->img_shoping==""||$x->img_shoping==" "||$x->img_shoping==NULL){
            $thumnail=json_decode($x->thumnail);
        $values['image_link'] = $thumnail->url;
        }else{
        $values['image_link'] = $x->img_shoping;
        }
        //
